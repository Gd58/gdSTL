...
public:
	...
	size_type _t_target(iterator lhs){
		iterator tmp = begin();
		size_type n = 0;
		while (tmp != lhs){
			++tmp;
	...
	bool _t_less(iterator lhs, iterator rhs){
		return _t_target(lhs) < _t_target(rhs);
	}
	bool _t_more(iterator lhs, iterator rhs){
		return _t_target(lhs) > _t_target(rhs);
	}
	/*
	void swap(iterator& lhs, iterator& rhs){
		auto tmp = lhs.node->data;
		auto _tmp = rhs.node->data;
		lhs.node->data = _tmp;
		rhs.node->data = tmp;
	}
	*/
